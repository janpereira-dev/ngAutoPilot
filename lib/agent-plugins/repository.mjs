...
export function resolveAngularInstallation({ root, projectRoot, target, profile = 'core', capabilities = [] }) {
  ...
  const requestedTarget = parseAngularTarget(target ?? {
    major: evidence.angular.major,
    minor: evidence.angular.minor,
  });
  ...
  if (!evidence.lockfile && requestedTarget.minor !== undefined) {
    ...
    if (!isTargetCompatibleWithDeclaration(coreDeclarationVersion, targetVersion)) {
      throw new Error(`Angular target ${formatTarget(requestedTarget)} is outside declared @angular/core ${coreDeclaration.version}`);
    }
  }
