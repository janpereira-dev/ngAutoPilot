...
export function resolveAngularInstallation({ root, projectRoot, target, profile = 'core', capabilities = [] }) {
  ...
  } else if (!evidence.lockfile && requestedTarget.minor !== undefined) {
  ...
  const compatibleSkills = evaluatedSkills.filter(({ compatibility }) => isAngularCompatible(compatibility, effectiveAngularMajor));
  const incompatibleSkills = evaluatedSkills.filter(({ compatibility }) => !isAngularCompatible(compatibility, effectiveAngularMajor));
...
function incompatibilityReason({ min, max }, major, label) {
