---
id: angular.upgrades.security.angular-v22-service-worker-sensitive-header-gate
name: Angular v22 Service Worker Sensitive Header Gate
description: >
  Use this skill for Review service worker redirects, referrer policy, and sensitive headers across origins. Use when Angular 22 work needs concern-first routing, explicit validation, and no invented APIs.
stack:
  - Angular
  - TypeScript
category: security
...
triggers:
  ...
  - sensitive headers
  - redirects
  - Angular v22 Service Worker Sensitive Header Gate
  - Security
  - Angular 22
compatibility:
  angular:
...
Review service worker redirects, referrer policy, and sensitive headers across origins.

Keep the work concern-first: this skill handles one risk area and does not replace the Angular 21 to 22 hop orchestrator.

## When to Use

- The task specifically involves review service worker redirects, referrer policy, and sensitive headers across origins.
...
Angular 22 changes security behavior enough that agents need a narrow checklist instead of a generic v22 bucket. The goal is to make the risk visible, testable, and reviewable.
...
- Use this skill to review `service worker` in an Angular 22 migration.
