---
id: angular.security.angular-token-storage-policy
name: Angular Token Storage Policy
description: Define a safe policy for storing and rotating auth tokens in Angular applications.
stack:
  - Angular
category: security
status: stable
...
Use this skill to handle Angular security review for angular token storage policy in Angular projects without mixing it with unrelated migration, modernization, or cleanup work.
...
- the task mentions angular token storage policy, XSS, DOM sanitization, token storage, SSR security, or dependency triage;
...
1. State the angular.security.angular-token-storage-policy diagnosis in one concise paragraph.
