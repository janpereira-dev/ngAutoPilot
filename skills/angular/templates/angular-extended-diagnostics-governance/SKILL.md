...
Use this skill to handle Angular template safety and diagnostics for angular extended diagnostics governance in Angular projects without mixing it with unrelated migration, modernization, or cleanup work.

This skill keeps the templates decision explicit: identify the current state, choose the smallest safe action, document compatibility evidence, and leave a validation path that another agent or maintainer can repeat.

## When to Use

Use this skill when:

- the task mentions angular extended diagnostics governance, strict templates, extended diagnostics, template type checking, or remediation;
- an Angular codebase needs a scoped templates decision before implementation;
...
- [ ] The templates scope is isolated from unrelated work.
...
1. State the angular.templates.angular-extended-diagnostics-governance diagnosis in one concise paragraph.
