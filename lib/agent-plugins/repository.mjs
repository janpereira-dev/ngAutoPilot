...
export function resolveAngularInstallation({ root, projectRoot, target, profile = 'core', capabilities = [] }) {
  ...
  const effectiveAngularMajor = requestedTarget.major;
  ...
  const compatibleSkills = evaluatedSkills.filter(({ skill, compatibility }) => isAngularCompatible(skill, compatibility, effectiveAngularMajor));
  const incompatibleSkills = evaluatedSkills.filter(({ skill, compatibility }) => !isAngularCompatible(skill, compatibility, effectiveAngularMajor));
  ...
  return {
    ...
    included: compatibleSkills.map(({ skill, packIds, compatibility }) => ({
      type: 'skill', id: skill.id, reason: skillReason(compatibilityReason(compatibility, effectiveAngularMajor), packIds),
    })),
    excluded: [
      ...incompatibleSkills.map(({ skill, packIds, compatibility }) => ({ type: 'skill', id: skill.id, reason: skillReason(incompatibilityReason(compatibility, effectiveAngularMajor, packageOnlyRange ? 'target' : 'detected'), packIds) })),
...
function incompatibilityReason(compatibility, major, label) {
  if (!compatibility || Object.keys(compatibility).length === 0) {
    return `requires an explicit Angular compatibility declaration; ${label} ${major}`;
  }
  const { min, max } = compatibility;
  if (min !== undefined && major < min) return `requires Angular >=${min}; ${label} ${major}`;
  return `requires Angular <=${max}; ${label} ${major}`;
}
