...
`security:scan` is a deterministic release gate. It scans every UTF-8 text file in the checkout that the public source-snapshot bundles can copy, including source skills, distributable plugin bundles, assets such as SVGs, agents, adapters, CLI and library code, configuration and schemas, Git hooks, packs, scripts, documentation, marketplace manifests, and workflow definitions. It deliberately excludes Git metadata, dependencies, generated `dist/` output, and local Skill Lab caches/runs. It rejects unresolved merge markers, invisible or bidirectional Unicode controls, remote shell and PowerShell execution pipelines, private-key or credential-shaped material, and broad `allowed-tools` shell permissions in skill frontmatter. It is defense in depth, not proof that prose is safe.
