---
id: angular.upgrades.di.angular-v22-service-decorator-adoption
name: Angular v22 Service Decorator Adoption
description: >
  Use this skill for Decide when @Service() is clearer than @Injectable({ providedIn: root }) and when Injectable remains required. Use when Angular 22 work needs concern-first routing, explicit validation, and no invented APIs.
stack:
  - Angular
  - TypeScript
...
triggers:
  ...
  - "@Injectable"
  - providedIn root
  - Angular v22 Service Decorator Adoption
...
Decide when @Service() is clearer than @Injectable({ providedIn: root }) and when Injectable remains required.

Keep the work concern-first: this skill handles one risk area and does not replace the Angular 21 to 22 hop orchestrator.

## When to Use

- The task specifically involves decide when @service() is clearer than @injectable({ providedin: root }) and when injectable remains required.
...
- Use this skill to review `@Service` in an Angular 22 migration.
