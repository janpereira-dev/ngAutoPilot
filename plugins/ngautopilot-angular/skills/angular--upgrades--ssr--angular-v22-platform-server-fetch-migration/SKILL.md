---
id: angular.upgrades.ssr.angular-v22-platform-server-fetch-migration
name: Angular v22 Platform Server Fetch Migration
description: >
  Use this skill for Replace deprecated platform-server XHR assumptions with fetch-compatible server behavior. Use when Angular 22 work needs concern-first routing, explicit validation, and no invented APIs.
stack:
  - Angular
  - TypeScript
category: ssr
...
triggers:
  ...
  - fetch
  - XHR deprecated
  - Angular v22 Platform Server Fetch Migration
  - SSR / Hydration
  - Angular 22
compatibility:
  angular:
...
Replace deprecated platform-server XHR assumptions with fetch-compatible server behavior.

Keep the work concern-first: this skill handles one risk area and does not replace the Angular 21 to 22 hop orchestrator.

## When to Use

- The task specifically involves replace deprecated platform-server xhr assumptions with fetch-compatible server behavior.
...
- Use this skill to review `platform-server` in an Angular 22 migration.
