---
id: angular.upgrades.change-detection.angular-v22-onpush-default-eager-migration
name: Angular v22 OnPush Default Eager Migration
description: >
  Use this skill for Control the Angular v22 change detection default shift and preserve Eager behavior only where required. Use when Angular 22 work needs concern-first routing, explicit validation, and no invented APIs.
stack:
  - Angular
  - TypeScript
...
triggers:
  ...
  - Angular v22 OnPush Default Eager Migration
...
Control the Angular v22 change detection default shift and preserve Eager behavior only where required.

Keep the work concern-first: this skill handles one risk area and does not replace the Angular 21 to 22 hop orchestrator.

## When to Use

- The task specifically involves control the angular v22 change detection default shift and preserve eager behavior only where required.
...
Angular 22 changes change detection / zoneless behavior enough that agents need a narrow checklist instead of a generic v22 bucket. The goal is to make the risk visible, testable, and reviewable.
...
- Use this skill to review `OnPush default` in an Angular 22 migration.
