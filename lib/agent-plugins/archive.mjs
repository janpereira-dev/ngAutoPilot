...
export async function createPluginArchives({ sourceRoot, outputRoot, version, pluginNames }) {
  fs.rmSync(outputRoot, { recursive: true, force: true });
  fs.mkdirSync(outputRoot, { recursive: true });
  const archives = [];

  for (const name of fs.readdirSync(sourceRoot).filter((candidate) => !pluginNames || pluginNames.includes(candidate)).sort()) {
...
function createZip(root, output) {
  return new Promise((resolve, reject) => {
    ...
    for (const file of listFiles(root)) {
      zip.addFile(file.absolute, file.relative, { mtime: zipDate, mode: 0o100644, compress: true, forceDosTimestamp: true });
    }
    zip.end();
  });
}
