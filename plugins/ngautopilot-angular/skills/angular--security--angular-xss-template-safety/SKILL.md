---
id: angular.security.angular-xss-template-safety
name: Angular XSS Template Safety
description: Prevent XSS by keeping templates, bindings, and trusted HTML usage safe and explicit.
stack:
  - Angular
category: security
status: stable
...
# Angular XSS Template Safety

## Purpose

Use this skill to handle Angular security review for angular xss template safety in Angular projects without mixing it with unrelated migration, modernization, or cleanup work.

This skill keeps the security decision explicit: identify the current state, choose the smallest safe action, document compatibility evidence, and leave a validation path that another agent or maintainer can repeat.

## When to Use

Use this skill when:

- the task mentions angular xss template safety, XSS, DOM sanitization, token storage, SSR security, or dependency triage;
- an Angular codebase needs a scoped security decision before implementation;
...
- [ ] The security scope is isolated from unrelated work.
...
1. State the angular.security.angular-xss-template-safety diagnosis in one concise paragraph.
