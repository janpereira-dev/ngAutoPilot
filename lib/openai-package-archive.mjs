...
export function rewriteLocalReferences(body, sourcePath, repositoryRoot, packageRoot, generatedSkillFile, publicPaths) {
  const rewritten = body.replace(/\]\(([^)\s]+)(\s+[^)]*)?\)/g, (whole, target, suffix = '') => {
    ...
    const packageTarget = publicResourceTarget(packageRoot, resolvedRepositoryRoot, safeSourceTarget);
...
function publicResourceTarget(packageRoot, repositoryRoot, sourceTarget) {
  const relative = path.relative(repositoryRoot, sourceTarget);
  const packageRelative = relative === 'skills' || relative.startsWith(`skills${path.sep}`)
    ? path.join('resources', relative)
    : relative;
  return path.join(packageRoot, packageRelative);
}
