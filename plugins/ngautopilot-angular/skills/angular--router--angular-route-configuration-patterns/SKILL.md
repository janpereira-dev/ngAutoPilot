---
id: angular.router.angular-route-configuration-patterns
name: Angular Route Configuration Patterns
description: Design route configuration with clean boundaries, lazy loading, and maintainable route metadata.
stack:
  - Angular
category: router
status: stable
...
Use this skill to handle Angular router design for angular route configuration patterns in Angular projects without mixing it with unrelated migration, modernization, or cleanup work.
...
- the task mentions angular route configuration patterns, route configuration, lazy loading, guards, resolvers, or router tests;
- an Angular codebase needs a scoped router decision before implementation;
...
- [ ] The router scope is isolated from unrelated work.
...
1. State the angular.router.angular-route-configuration-patterns diagnosis in one concise paragraph.
