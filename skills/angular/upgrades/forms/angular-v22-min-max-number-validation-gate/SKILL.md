---
id: angular.upgrades.forms.angular-v22-min-max-number-validation-gate
name: Angular v22 Min Max Number Validation Gate
description: >
  Use this skill for Audit min/max number validators so numeric contracts are not accidentally passed as strings. Use when Angular 22 work needs concern-first routing, explicit validation, and no invented APIs.
stack:
  - Angular
  - TypeScript
category: forms
...
triggers:
  ...
  - max
  - number validation
  - Angular v22 Min Max Number Validation Gate
  - Forms / Signal Forms
  - Angular 22
compatibility:
  angular:
...
Audit min/max number validators so numeric contracts are not accidentally passed as strings.

Keep the work concern-first: this skill handles one risk area and does not replace the Angular 21 to 22 hop orchestrator.

## When to Use

- The task specifically involves audit min/max number validators so numeric contracts are not accidentally passed as strings.
...
- Use this skill to review `min` in an Angular 22 migration.
