...
export function createZip(root, output) {
  return new Promise((resolve, reject) => {
    ...
    const fail = (error) => {
      if (settled) return;
      settled = true;
      stream.once('close', removeIncompleteOutput);
...
function readZipEntries(file, errors) {
  ...
  for (let index = 0; index < totalEntries; index += 1) {
    ...
    if (!local) return [];
    if (!validateZipPayload(data, local.payloadStart, local.payloadEnd, method, data.readUInt32LE(offset + 16), uncompressedSize, name, errors)) return [];
...
function readLocalRecord(data, centralEntry, errors) {
  ...
  return { payloadStart: localEnd, payloadEnd };
}
...
function validateZipPayload(data, payloadStart, payloadEnd, method, expectedCrc32, expectedUncompressedSize, name, errors) {
  ...
  try {
    uncompressed = method === 0
      ? payload
      : zlib.inflateRawSync(payload, { maxOutputLength: Math.max(1, Math.min(MAX_ARCHIVE_FILE_BYTES, expectedUncompressedSize + 1)) });
  } catch {
