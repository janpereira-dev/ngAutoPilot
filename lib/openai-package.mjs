...
export async function buildOpenAiPackage({ root = process.cwd(), outputRoot, version = readPackageVersion(root) } = {}) {
  ...
  prepareOutputRoot(resolvedOutputRoot);
...
function prepareOutputRoot(outputRoot) { if (fs.existsSync(outputRoot)) fs.rmSync(outputRoot, { recursive: true, force: true }); fs.mkdirSync(outputRoot, { recursive: true }); }
