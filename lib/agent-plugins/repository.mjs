...
const angularCatalogSupport = Object.freeze({ min: 2, max: 22, unsupported: new Set([3]) });
const requiredSkillSections = Object.freeze(['## Purpose', '## When to Use', '## Do', '## Do Not', '## Review Checklist', '## Expected Output']);
...
export function createRepositoryTools({ root }) {
  return Object.freeze({
    ...
    catalogQuality: () => catalogQuality(root),
    packList: () => listPacks(root),
    packResolve: ({ packId }) => resolvePack(root, packId),
    platformInventory: () => platformInventory(root),
    ...
    angularInstallationResolve: (options) => resolveAngularInstallation({ root, ...options }),
...
export function resolveAngularInstallation({ root, projectRoot, target, profile = 'core', capabilities = [] }) {
  ...
  assertSupportedAngularMajor(evidence.angular.major, 'detected');
  ...
  assertSupportedAngularMajor(requestedTarget.major, 'requested');

  const packageOnlyRange = !evidence.lockfile && coreDeclarationVersion.operator !== '';
  if (!packageOnlyRange && (requestedTarget.major !== evidence.angular.major
  ...
  const satisfiablePackageOnlyMinor = packageOnlyRange && requestedTarget.minor === undefined
    ? findSatisfiableAngularMinor(requestedTarget.major, evidence.angular.declarations)
    : undefined;
  if (packageOnlyRange && requestedTarget.minor === undefined) {
    if (satisfiablePackageOnlyMinor === undefined) {
      throw new Error(`Angular target ${formatTarget(requestedTarget)} has no satisfiable minor across declared Angular ranges`);
    }
  } else if (requestedTarget.minor !== undefined) {
  ...
  const effectiveAngularTarget = requestedTarget.minor === undefined
    ? { ...requestedTarget, minor: packageOnlyRange ? satisfiablePackageOnlyMinor : evidence.angular.minor }
    : requestedTarget;
  ...
  const evaluatedSkills = [...selectedSkills.values()]
    ...
    .map(({ skill, packIds }) => ({ skill, packIds, compatibility: readAngularCompatibility(repositoryRoot, skill) }));
  const compatibleSkills = evaluatedSkills.filter(({ skill, compatibility }) => isAngularCompatible(skill, compatibility, effectiveAngularTarget));
  const incompatibleSkills = evaluatedSkills.filter(({ skill, compatibility }) => !isAngularCompatible(skill, compatibility, effectiveAngularTarget));

  return {
    projectRoot: project.root,
    target: effectiveAngularTarget,
    profile,
    ...
    selection: {
      sourcePacks: packs.map((pack) => ({
        id: pack.id,
        reason: requestedPacks.includes(pack.id) ? 'requested profile or capability' : 'transitive pack dependency',
      })),
      installable: false,
      reason: 'This is a filtered selection report. Named packs remain unfiltered and must not be installed directly when this report excludes skills.',
    },
    included: compatibleSkills.map(({ skill, packIds, compatibility }) => ({
      type: 'skill', id: skill.id, reason: skillReason(compatibilityReason(compatibility, effectiveAngularTarget), packIds),
    })),
    excluded: [
      ...incompatibleSkills.map(({ skill, packIds, compatibility }) => ({ type: 'skill', id: skill.id, reason: skillReason(incompatibilityReason(compatibility, effectiveAngularTarget, packageOnlyRange ? 'target' : 'detected'), packIds) })),
...
function assertSupportedAngularMajor(major, source) {
  if (major < angularCatalogSupport.min || major > angularCatalogSupport.max || angularCatalogSupport.unsupported.has(major)) {
    throw new Error(`unsupported ${source} Angular major ${major}; this catalog supports Angular 2 and Angular 4 through 22`);
  }
}

function inspectContentSignals(content) {
  const missing = missingRequiredSections(content);
  return {
    requiredSections: missing.length === 0,
...
function missingRequiredSections(content) {
  if (typeof content !== 'string') {
    throw new Error('source skill content is required when the catalog has no content signals');
  }
  return requiredSkillSections.filter((section) => !hasExactHeading(content, section));
}
...
/**
 * Return content signals for every source skill. These signals are intentionally
 * not a semantic-quality score: only controlled evaluation in Skill Lab and
 * human review can establish whether guidance is useful in practice.
 */
export function catalogQuality(root) {
  const skills = readCatalog(root).skills ?? [];
  const evaluated = skills.map((skill) => {
    const cachedSignals = skill.contentSignals;
    const requiresSource = !cachedSignals
      || (!cachedSignals.requiredSections && !Array.isArray(cachedSignals.missingRequiredSections));
    const sourcePath = path.join(root, skill.path);
    const content = requiresSource && fs.existsSync(sourcePath) ? fs.readFileSync(sourcePath, 'utf8') : undefined;
    const signals = cachedSignals ?? inspectContentSignals(content);
    const missingSections = signals.missingRequiredSections
      ?? (typeof content === 'string' ? missingRequiredSections(content) : []);
    const reviewNeeded = signals.requiredSections ? [] : ['missing-required-sections'];
    return { id: skill.id, path: skill.path, signals, missingSections, reviewNeeded };
  });
  const reviewNeeded = evaluated.filter((skill) => skill.reviewNeeded.length > 0);
  return {
    scope: 'deterministic-content-signals',
    semanticEvaluation: 'Use Skill Lab benchmark, adversarial split, and human review before promotion; this inventory does not claim semantic value.',
    summary: {
      skillCount: evaluated.length,
      allRequiredSectionsPresent: reviewNeeded.length === 0,
      reviewNeededCount: reviewNeeded.length,
      totalWords: evaluated.reduce((total, skill) => total + skill.signals.wordCount, 0),
    },
    skills: evaluated,
  };
}

export function platformInventory(root) {
  const catalog = readCatalog(root);
  const packs = listPacks(root);
  const skillFamilies = new Map();
  for (const skill of catalog.skills ?? []) {
    const family = skill.id.split('.')[0];
    skillFamilies.set(family, (skillFamilies.get(family) ?? 0) + 1);
  }
  const adaptersRoot = path.join(root, 'adapters');
  const adapters = fs.readdirSync(adaptersRoot, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && entry.name !== '_shared')
    .map((entry) => readJson(path.join(adaptersRoot, entry.name, 'manifest.json')))
    .sort((left, right) => left.id.localeCompare(right.id));
  const subagentsRoot = path.join(root, 'agents', 'ngautopilot', 'subagents');
  const subagents = listMarkdownFiles(subagentsRoot)
    .map((file) => ({
      id: path.basename(file, '.md').replace(/^\d+-/, ''),
      path: toPosixPath(path.relative(root, file)),
      name: firstHeading(fs.readFileSync(file, 'utf8')),
    }))
    .sort((left, right) => left.id.localeCompare(right.id));

  const packageMetadata = readJson(path.join(root, 'package.json'));
  const distribution = packageMetadata.ngautopilot?.distribution;
  if (!distribution) throw new Error('package.json is missing the canonical ngautopilot distribution metadata');

  return {
    version: packageMetadata.version,
    skills: {
      count: (catalog.skills ?? []).length,
      families: Object.fromEntries([...skillFamilies.entries()].sort(([left], [right]) => left.localeCompare(right))),
    },
    angular: {
      supportedMajorRange: { min: angularCatalogSupport.min, currentCatalog: angularCatalogSupport.max },
      unsupportedMajor: [...angularCatalogSupport.unsupported],
      upgradeHops: packs
        .map(({ id }) => id.match(/^ngautopilot-angular-(\d+)-to-(\d+)$/))
        .filter(Boolean)
        .map((match) => ({ from: Number(match[1]), to: Number(match[2]) }))
        .sort((left, right) => left.from - right.from),
      extensionRule: 'Add a version-specific compatibility declaration, pack or hop, tests, and documentation; do not rewrite prior-version assets.',
    },
    packs: { count: packs.length, ids: packs.map(({ id }) => id) },
    adapters,
    subagents,
    distribution,
    quality: catalogQuality(root).summary,
  };
}
...
function readAngularEvidence(project) {
  ...
  for (const declaration of angularDeclarations) {
    if (lockedVersion && !isDeclaredVersionSatisfied(declaration.version, lockedVersion)) {
      throw new Error(`package.json ${declaration.name} ${declaration.version.raw} contradicts lockfile ${lockedVersion.raw}`);
    }
  }
...
function deriveOmittedAngularTarget({ major, minor: detectedMinor, declarations }) {
  const firstSatisfiableMinor = findSatisfiableAngularMinor(major, declarations);
...
function findSatisfiableAngularMinor(major, declarations) {
  const parsedDeclarations = declarations.map(({ version }) => parseDeclaredVersion(version));
  const candidateMinors = [...new Set([0, ...parsedDeclarations.flatMap((declaration) => [
    declaration.minor,
    ...(declaration.operator === '>' && !declaration.hasPatch ? [declaration.minor + 1] : []),
  ])])].sort((left, right) => left - right);
  return candidateMinors.find((candidateMinor) => parsedDeclarations.every((declaration) => (
...
function readAngularCompatibility(repositoryRoot, skill) {
  if (skill.compatibility) return skill.compatibility.angular ?? skill.compatibility;
...
function isAngularCompatible(skill, compatibility, target) {
  if (!compatibility || Object.keys(compatibility).length === 0) return !skill.id.startsWith('angular.');
  const { min, minMinor, max, maxMinor } = compatibility;
  return (min === undefined || compareAngularTarget(target, min, minMinor) >= 0)
    && (max === undefined || compareAngularTarget(target, max, maxMinor) <= 0);
}

function compatibilityReason({ min, minMinor, max, maxMinor }, target) {
  if (min === undefined && max === undefined) return 'no declared Angular compatibility bound';
  if (min !== undefined && max !== undefined) return `compatible with Angular ${formatTarget(target)}; declared range ${formatAngularBound(min, minMinor)}-${formatAngularBound(max, maxMinor)}`;
  return min !== undefined
    ? `compatible with Angular ${formatTarget(target)}; declared minimum ${formatAngularBound(min, minMinor)}`
    : `compatible with Angular ${formatTarget(target)}; declared maximum ${formatAngularBound(max, maxMinor)}`;
}
...
function incompatibilityReason(compatibility, target, label) {
  if (!compatibility || Object.keys(compatibility).length === 0) {
    return `requires an explicit Angular compatibility declaration; ${label} ${formatTarget(target)}`;
  }
  const { min, minMinor, max, maxMinor } = compatibility;
  if (min !== undefined && compareAngularTarget(target, min, minMinor) < 0) {
    return `requires Angular >=${formatAngularBound(min, minMinor)}; ${label} ${formatTarget(target)}`;
  }
  return `requires Angular <=${formatAngularBound(max, maxMinor)}; ${label} ${formatTarget(target)}`;
}

function compareAngularTarget(target, boundMajor, boundMinor = 0) {
  if (target.major !== boundMajor) return target.major - boundMajor;
  return (target.minor ?? 0) - boundMinor;
}

function formatAngularBound(major, minor) {
