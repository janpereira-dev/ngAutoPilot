...
export function catalogQuality(root) {
  ...
  const evaluated = skills.map((skill) => {
    const content = skill.contentSignals ? undefined : fs.readFileSync(path.join(root, skill.path), 'utf8');
    const signals = skill.contentSignals ?? {
    ...
    const missingSections = signals.requiredSections ? [] : requiredSections.filter((section) => !content.includes(section));
    return { id: skill.id, path: skill.path, signals, reviewNeeded: missingSections.length > 0 ? ['missing-required-sections'] : [] };
  });
...
export function platformInventory(root) {
  ...
  return {
    ...
    angular: {
      supportedMajorRange: { min: 2, currentCatalog: 22 },
      unsupportedMajor: [3],
      upgradeHops: packs
    ...
    subagents,
    distribution: {
      claudeMarketplace: fs.existsSync(path.join(root, '.claude-plugin', 'marketplace.json')),
      codexMarketplace: fs.existsSync(path.join(root, '.agents', 'plugins', 'marketplace.json')),
      openaiPackage: fs.existsSync(path.join(root, 'openai', 'plugin.json')),
      mcpServer: fs.existsSync(path.join(root, 'mcp', 'server-entry.mjs')),
    },
