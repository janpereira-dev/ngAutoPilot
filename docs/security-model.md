...
`security:scan` is a deterministic release gate. It scans source skills, distributable plugin bundles, agents, adapters, CLI and library code, configuration, packs, scripts, published documentation, marketplace manifests, and workflow definitions. It rejects unresolved merge markers, invisible or bidirectional Unicode controls, remote shell and PowerShell execution pipelines, private-key or credential-shaped material, and broad `allowed-tools` shell permissions in skill frontmatter. It is defense in depth, not proof that prose is safe.
