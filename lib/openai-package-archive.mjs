...
function readZipEntries(file, errors) {
  ...
  for (const centralEntry of centralEntries) {
    const local = readLocalRecord(data, centralEntry, errors);
    if (!local) return [];
    if (!validateZipPayload(data, local.payloadStart, local.payloadEnd, centralEntry.method, centralEntry.crc32, centralEntry.uncompressedSize, centralEntry.name, errors)) return [];
    entries.push({
