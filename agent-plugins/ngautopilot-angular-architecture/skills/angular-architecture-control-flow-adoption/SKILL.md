---
name: angular-architecture-control-flow-adoption
description: "Adopt Angular control flow syntax in bounded post-upgrade modernization slices to replace structural directive boilerplate with `@if`, `@for`, and `@switch` without mixing the change into an Angular version hop."
license: MIT
metadata:
