---
id: angular.upgrades.router.angular-v22-params-inheritance-always-gate
name: Angular v22 Params Inheritance Always Gate
description: >
  Use this skill for Audit routes that depend on params/data inheritance so inherited IDs do not silently change behavior. Use when Angular 22 work needs concern-first routing, explicit validation, and no invented APIs.
stack:
  - Angular
  - TypeScript
category: router
...
triggers:
  ...
  - route params
  - route data
  - Angular v22 Params Inheritance Always Gate
  - Router
  - Angular 22
compatibility:
  angular:
...
Audit routes that depend on params/data inheritance so inherited IDs do not silently change behavior.

Keep the work concern-first: this skill handles one risk area and does not replace the Angular 21 to 22 hop orchestrator.

## When to Use

- The task specifically involves audit routes that depend on params/data inheritance so inherited ids do not silently change behavior.
...
- Use this skill to review `paramsInheritanceStrategy` in an Angular 22 migration.
